import pygame

class Menu:
    def __init__(self, screen, titulo, opciones):
        self.screen = screen
        self.titulo = titulo
        self.opciones = opciones
        self.selected_index = 0

        # Fuentes (Usamos None para evitar errores si falta el archivo .ttf)
        try:
            self.font = pygame.font.Font("ui/fonts/early_gameboy.ttf", 48)
            self.title_font = pygame.font.Font("ui/fonts/early_gameboy.ttf", 50)
        except:
            # Si no encuentra la fuente, usa la predeterminada
            self.font = pygame.font.Font(None, 48)
            self.title_font = pygame.font.Font(None, 50)

        # Colores
        self.color_normal = (116,103,193)
        self.color_selected = (255,220,100)
        self.bg_color = (46,42,79)

        self.buttons = []
        self._create_buttons()
        
        # Imagen de fondo (Manejo de error por si no existe la imagen)
        self.bg_image = None
        try:
            loaded_img = pygame.image.load("ui/image/clouds_2.png").convert()
            self.bg_image = pygame.transform.scale(loaded_img, (self.screen.get_width(), self.screen.get_height()))
        except:
            pass # Si falla, simplemente no habrá imagen de fondo
        
        # Parametros de animacion para el fondo
        self.bg_x = 0
        self.bg_speed = 0.3

    def _create_buttons(self):
        center_x = self.screen.get_width() // 2
        start_y = self.screen.get_height() // 2

        for i, texto in enumerate(self.opciones):
            rect = pygame.Rect(0, 0, 450, 60)
            rect.center = (center_x, start_y + i * 120)
            self.buttons.append((texto, rect))

    def render(self):
        # 1. Dibujar Fondo
        if self.bg_image:
            self.bg_x -= self.bg_speed
            if self.bg_x <= -self.screen.get_width():
                self.bg_x = 0
            self.screen.blit(self.bg_image, (self.bg_x, 0))
            self.screen.blit(self.bg_image, (self.bg_x + self.screen.get_width(), 0))
        else:
            self.screen.fill(self.bg_color) # Fondo sólido si no hay imagen

        self._draw_title()
        self._draw_buttons()

    def _draw_title(self):
        title_surf = self.title_font.render(self.titulo, True, (255, 255, 255))
        title_rect = title_surf.get_rect(center=(self.screen.get_width() // 2, 120))
        self.screen.blit(title_surf, title_rect)
    
    def _draw_buttons(self):
        for i, (texto, rect) in enumerate(self.buttons):
            # Color del botón
            if i == self.selected_index:
                # Parpadeo cada 500ms
                color = (255, 220, 100) if pygame.time.get_ticks() % 500 < 250 else (243, 181, 47)
            else:
                color = self.color_normal

            # Animación de tamaño del botón 
            draw_rect = rect.copy() 
            if i == self.selected_index:
                draw_rect.inflate_ip(10, 5)

            # Dibujar rectángulo 
            pygame.draw.rect(self.screen, color, draw_rect, border_radius=8)

            # Dibujar texto centrado 
            text_surf = self.font.render(texto, True, (0, 0, 0))
            text_rect = text_surf.get_rect(center=draw_rect.center)
            self.screen.blit(text_surf, text_rect)

    # --- ESTA ES LA FUNCIÓN QUE FALTABA O ESTABA MAL INDENTADA ---
    def update(self, events):
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_DOWN:
                    self.selected_index = (self.selected_index + 1) % len(self.buttons)

                elif event.key == pygame.K_UP:
                    self.selected_index = (self.selected_index - 1) % len(self.buttons)

                elif event.key == pygame.K_RETURN:
                    return self.opciones[self.selected_index]
        return None
    # -------------------------------------------------------------

# CLASES HIJAS (Heredan todo lo de arriba, incluido 'update')

class MenuInicial(Menu):
    def __init__(self, screen):
         super().__init__(
             screen,
             "Penguin Chaos",
             ["Jugar", "Salir"]
         )

class Menu_GameOver(Menu):
    def __init__(self, screen):
        super().__init__(
            screen,
            "Game Over",
            ["Reiniciar", "Salir"]
        )        

class Menu_Pausa(Menu):
    def __init__(self, screen):
        super().__init__(
            screen,
            "Juego en Pausa",
            ["Reanudar", "Salir"]
        )