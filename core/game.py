...
from systems.powerups_spawner import PowerUpSpawner
...
class Game:
    def __init__(self, screen):
        ...
        self.spawn_system.spawn_initial(self.students, self._get_free_seat, self.exit_position)
        # dificultad/caos global
        self.chaos_system = ChaosSystem(config)
        
        # Power-ups
        self.powerups = []  
        self.powerup_system = PowerUpSystem(self.player, self.students, self.chaos_system)
        self.powerup_spawner = PowerUpSpawner(self.player, self.students, self.obstacles)
    ...
    # ─────────────────────────────
    def update(self, dt):
        # process input each frame so `interact_pressed` is up-to-date
        ...
        if self.powerup_spawn_timer >= self.powerup_spawn_interval:
            if len(self.powerups) < 3:  # Máximo 3 power-ups
                new_powerup = self.powerup_spawner.spawn(self.screen.get_rect())
                self.powerups.append(new_powerup)
            self.powerup_spawn_timer = 0
        
        # Remover power-ups inactivos
