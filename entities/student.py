...
class Student(pygame.sprite.Sprite):
    def __init__(self, spawn_pos, seat_pos, exit_pos, seat_index=None, get_free_seat=None):
        ...
        self.frozen = False
        self.saved_move_direction = pygame.Vector2(0, 0)
    ...
    def freeze(self):
        """Congela al estudiante (guarda su estado actual)"""
        if not self.frozen:
            self.frozen = True
            self.saved_move_direction = self.move_direction.copy()
            self.move_direction = pygame.Vector2(0, 0)

    def unfreeze(self):
        """Descongela al estudiante (restaura su comportamiento)"""
        if self.frozen:
            self.frozen = False
            # Si estaba caminando, restaurar dirección
    ...
    def render(self, screen):
        ...
        # Efecto visual cuando está congelado (opcional)
        if self.frozen:
            # Dibuja un tinte azul semi-transparente sobre el estudiante
            frozen_overlay = pygame.Surface(self.rect.size, pygame.SRCALPHA)
            frozen_overlay.fill((150, 200, 255, 100))  # Azul claro transparente
            screen.blit(frozen_overlay, self.rect.topleft)
